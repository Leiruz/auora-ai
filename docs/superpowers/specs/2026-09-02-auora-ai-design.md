...
| | |
|---|---|
| Status | Draft v0.3. Rounds 1 and 2 of the Codex adversarial review are recorded in section 16; the round-2 findings are resolved in v0.3 by design but that resolution is not yet cross-reviewed, because the review harness caps plan reviews at two rounds per thread |
| Date | 2026-09-02 |
...
Auora AI is a locally installed, open-source **stateful action firewall for AI coding agents**. It starts each agent run (Claude Code, Codex CLI, or any command) inside an operating-system sandbox, turns every tool call the agent proposes into a typed descriptor, evaluates a deterministic policy that answers allow, throttle, require approval, deny or terminate, forces all name resolution and network traffic through its own resolver and proxy, injects scoped credentials at the proxy so agent processes never hold secrets, runs the JavaScript an agent writes inside a V8 isolate, and appends every decision to a signed, hash-chained log. A dashboard the user deploys to their own Cloudflare free account shows runs, decisions and behavior signals, and relays approvals to a phone that signs them with its own key. Version 1.0 is an open-source core launch on Windows, macOS and Linux; a hosted multi-tenant tier is the later commercial product.

This document opens with a reality check because the founder asked for one, then defines the product, its architecture, contracts, security model, requirements, tests and delivery plan. Changes from v0.1 and v0.2 are listed in section 16.

## 1. Reality check

### 1.1 Verdict

**Technically achievable, and buildable by one person with AI coding agents**, because the hard primitives exist and are maintained by others: Anthropic's open-source sandbox-runtime for the operating-system sandboxes, the agents' own hook interfaces for interception, Cloudflare's workerd for V8 isolates, and the Cloudflare free plan for the dashboard. The honest planning basis, adopted from the adversarial review, is **39 to 61 focused weeks**, which is 14 to 22 calendar months alongside NUS and the internship, because five of the seven sub-projects are new trust boundaries that need adversarial testing on three operating systems, and because the founder has chosen native Windows as a hard 1.0 requirement, which means Auora must build its own resolver control where the upstream sandbox does not fence DNS.
...
- Not taint tracking of arbitrary child processes. Data labels cover what Auora observed being read through a hook, the proxy or the isolate. The primary secret control is therefore that files matching secret patterns are unreadable inside the sandbox at all (section 6.6); labels are the second line, and anything unlabelled is governed by destination scope only.
...
| Gate | Criterion |
|---|---|
| Before launch | Protected mode passes the entire hostile corpus, including resolver control, on native Windows, macOS and Linux in CI; the three golden runs ship as transcripts |
...
| OS targets | Windows, macOS and Linux from day one; **native Windows is a hard 1.0 requirement**: 1.0 does not ship until native Windows passes the full hostile corpus, including the resolver control Auora builds itself | Founder decision after review finding F10 |
| Effort basis | 39 to 61 focused weeks (the reviewer's estimate), 14 to 22 calendar months | Founder decision after review finding F11 |
...
1. "Auora contains your agents with OS-level sandboxing and runs the code they write inside V8 isolates."
2. "Every tool call the agent proposes, every connection it opens and every name it resolves passes a deterministic policy you can read, simulate and version. The same input always gives the same answer."
3. "Agents never hold your secrets. Secret files are unreadable inside the sandbox, anything Auora has labelled secret cannot leave through any door, and scoped credentials are injected at Auora's proxy only for the hosts, methods and paths you allow, never for anything that moves money or mints credentials."
...
5. "Every decision is in a signed, hash-chained log you can verify offline."
6. "A run is labelled protected only when Auora can prove the sandbox, hooks, resolver and proxy are all active; anything less is labelled observed."
...
| 6 | Isolate runner | For each execution, spawn a disposable workerd child process inside the sandbox under an OS memory and process cap (a Job Object with a process-memory limit on Windows, a cgroup v2 `memory.max` or `prlimit` on Linux, `RLIMIT_AS` and the Seatbelt profile on macOS); run model-written JavaScript in it with `globalOutbound: null`, `disallow_eval_during_startup` pinned, CPU and subrequest limits where enforced, a daemon-enforced wall-clock timeout that terminates the whole process group, and capability bindings implemented as `WorkerEntrypoint` stubs that call the daemon; exposed to agents as an MCP server with `search`, `describe`, `execute`, `upload` | MCP over stdio | workerd binary, daemon untrusted channel |
...
- Windows: name resolution is performed by the host's DNS Client service, which Windows Filtering Platform cannot attribute to the sandbox user and which answers from a machine-wide cache. Auora therefore makes protected runs on Windows **exclusive**: at most one protected run at a time, so every lookup during the run belongs to that run by construction. At run start Auora, with administrator rights, flushes the DNS Client cache, installs a Name Resolution Policy Table rule directing all resolution to Auora's resolver, and installs WFP rules that block direct port 53 and DNS-over-HTTPS egress from the sandbox user; Auora's resolver answers with a zero time-to-live so the DNS Client caches nothing; the rules are removed at run end and on daemon crash recovery. A second `auora run` on Windows while a protected run is active is refused with the reason, or may start as `observed` if the user says so. The claim is stated precisely: on Windows every lookup during a protected run reaches Auora's resolver and is attributed to that run by exclusivity, not by process identity. This is machine-wide while a run is active and the README says so. It is the cost of the founder's decision that native Windows is a hard 1.0 requirement.
...
- Vault hosts are not exempt from the secret guard. Labelled secret content and bodies matching secret shapes are denied to every vault path unless the entry is explicitly marked `secret_bearing` (a secrets-manager API whose purpose is to receive secrets) and the request matches that entry's allowed path; such entries require a desktop approval to create.
...
| Daemon crash during a protected run on Windows | Recovery on next start removes the Name Resolution Policy Table rule and WFP rules; the launcher refuses new protected runs until recovery completes |
...
| Isolate allocates until memory is exhausted | The operating-system cap on the disposable child process kills it; the daemon records an out-of-memory code; other executions and the daemon are unaffected because each execution has its own process |
...
| Second protected run requested on Windows | Refused with the reason, or started as `observed` on request; exclusivity is what makes lookups attributable |
...
- Resolver: filtering resolver with per-OS enforcement (section 6.3); logs every lookup as an action; on Windows enforces exclusivity, flushes the cache at run start and answers with a zero time-to-live.
...
- Isolate runner: one disposable child process per execution under OS memory and process caps, `execute` with a wall-clock budget that terminates the process group, `search`, `describe` and `upload` over the run's capability catalog; capability stubs for the file, git and HTTP capabilities the project profile grants; `disallow_eval_during_startup` pinned.
...
- Secrets: default read-deny list for secret files verified before `protected`; upload capability that snapshots and digest-binds file bytes; secret-shape body scanning on vault requests.
...
| Portability | Windows 11, macOS 13+, Ubuntu 22.04+ and equivalents; native Windows must pass the full corpus before 1.0 | Three-OS CI matrix |
...
**Cross-OS matrix.** GitHub Actions on Ubuntu (bubblewrap), macOS (Seatbelt) and Windows (restricted user plus Windows Filtering Platform and the Name Resolution Policy Table; runners are administrators), free for a public repository, running the hostile corpus inside each real sandbox on every pull request.
...
Each sub-project gets its own spec, plan, test-first implementation, Codex cross-review, pull request and tag, following the founder's AI-enabled SDLC (issue first, PR, review before merge). Effort follows the adopted 39 to 61 week basis.
...
| 3 | Sandbox adapter, resolver with per-OS enforcement, proxy domain policy, secret-file denial | Protected mode on three OSes including native Windows | Hostile corpus green in the three-OS matrix, DNS exfiltration, Windows exclusivity and cache tests included; coverage labels correct | 8 to 12 |
...
| 6 | Isolate runner and MCP server | V8 isolates for model-written code | Isolate fixtures green including busy-loop abort, allocation bomb and fork bomb under OS caps; upload capability binds bytes; MCP tools usable from both agents | 3 to 5 |
...
Total 39 to 61 focused weeks; 14 to 22 calendar months alongside NUS and the internship. Sub-projects 3 and 4 can run in parallel once 2 is stable. A 0.9 preview for early users is possible after sub-project 4 on the operating systems that pass the corpus at that point, labelled accordingly; 1.0 waits for native Windows.
...
| sandbox-runtime's Windows backend is Alpha and does not fence DNS | Auora builds resolver control (section 6.3) and tests it in CI; 1.0 waits for native Windows to pass; there is no scope fallback, by founder decision |
...
| Windows machine-wide resolver redirection surprises users | Loud banner at run start, removed at run end and on crash recovery, documented in the README |
...
| Windows exclusivity frustrates users who run several agents at once | Documented; `observed` mode for the extra runs; a per-process DNS client is a future isolation track, not 1.0 |
...
| Re-estimate after the first three-OS trust boundary | Founder, on sub-project 3 evidence | End of sub-project 3 |
...
Not run. The tri harness caps plan reviews at two rounds per thread and refused a third (`verdict=cap-reached`), whose rule is to hand the open items to the founder rather than iterate. Status of the v0.3 resolutions: designed and written, not cross-reviewed. Options recorded for the founder: start a fresh-thread review of v0.3 (a new two-round budget), or proceed to sub-project 1 and let the sub-project's own plan review revisit the affected sections (5.3, 5.6, 6.3, 6.6, 8.2, unit 6) when each is implemented. The round-2 findings most worth a second opinion before build are F5 (secret guard) and N2 (per-execution process containment), because both change the trust boundary.
