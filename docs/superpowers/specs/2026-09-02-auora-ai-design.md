...
Rules that match on labels or signals (those with `labels_any`, `labels_read_any`, or `signals_any`) do not participate in the priority contest. Instead, they are evaluated separately, and any matching gated rule can only raise the outcome that the ungated rules (those without label or signal matchers) produced. This design ensures that adding labels to a descriptor or recording behavior signals never moves a decision towards allow, making evaluation monotone under labels and signals. Consequently, no label or signal can override an ungated rule at any priority, and when two gated rules match with different outcomes, the higher priority wins without producing a POLICY_CONFLICT.

Two guard postures were decided by the founder after the sub-project 1 reviews (issue #7) and are broader than the wording above. First, the protected-configuration guard covers the whole of the `.claude`, `.codex` and `.auora` directories, for writes and deletes alike: an agent cannot author skills, commands or agent definitions there, because those can run shell, and no bundle can open them; observe mode in sub-project 2 will show how often real runs hit this, and it may be narrowed later on that evidence. Second, once a run has read secret-labelled data, the guard denies every later external-scope action in that run, not only sends and lookups: in protected mode secret files are unreadable in the sandbox, so the secret label arrives from vault responses, after which any external action is the exfiltration shape. Both fail safe and both are recorded so that a later reader does not revert them as bugs.
